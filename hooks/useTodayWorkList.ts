...
import { isOverdue } from '../constants/status';
import { fetchAllWorkOrders } from '../lib/workOrders';

function sortWorkList(orders: WorkOrder[]): WorkOrder[] {
  return [...orders].sort((a, b) => {
    const aOverdue = isOverdue(a.delivery_date, a.status);
    const bOverdue = isOverdue(b.delivery_date, b.status);
