...
import { fetchProvas, createProva, updateProva } from '../../lib/provas';
import { fetchOrderItems } from '../../lib/services';
import { Prova, OrderItem, ProvaType, PROVA_TYPES } from '../../lib/types';
                      ...
                      <View style={s.quotaBar}>
                        {Array.from({ length: prova.quota }).map((_, i) => (
                          <View
                            ...
                            style={[
                              s.quotaDot,
                              i < doneCount && { backgroundColor: Colors.primary },
                            ]}
                          />
                        ))}
                      </View>
...
const s = StyleSheet.create({
  ...
  planBtnText: { color: Colors.white, fontWeight: '700', fontSize: 13 },
  ...
  provaNum: { fontSize: 14, fontWeight: '800', color: Colors.textPrimary },
  ...
  dateChipLabel: { fontSize: 10, color: Colors.textMuted, fontWeight: '500' },
  dateChipValue: { fontSize: 12, fontWeight: '700', color: Colors.textPrimary },
  quotaRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 },
  quotaLabel: { fontSize: 11, color: Colors.textMuted, fontWeight: '600', width: 32 },
  quotaBar: { flexDirection: 'row', gap: 4 },
  quotaDot: {
    width: 14, height: 14, borderRadius: 7,
    backgroundColor: Colors.border, borderWidth: 1, borderColor: Colors.border,
  },
  quotaCount: { fontSize: 12, fontWeight: '700', color: Colors.textPrimary },
  noteBlock: { marginBottom: 8 },
  noteLabel: { fontSize: 11, color: Colors.textMuted, fontWeight: '600', marginBottom: 3 },
  noteValue: { fontSize: 13, color: Colors.textPrimary, lineHeight: 18 },
  notePlaceholder: { fontSize: 13, color: Colors.textMuted, fontStyle: 'italic' },
  editRow: { gap: 6 },
  noteInput: {
    borderWidth: 1, borderColor: Colors.border, borderRadius: 8,
    padding: 10, fontSize: 13, color: Colors.textPrimary,
    backgroundColor: Colors.background, minHeight: 60, textAlignVertical: 'top',
  },
...
const modal = StyleSheet.create({
  ...
  close: { fontSize: 18, color: Colors.textMuted, fontWeight: '600', padding: 4 },
